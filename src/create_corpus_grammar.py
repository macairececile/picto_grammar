...
def clean_commonvoice(text):
    text_1 = text.replace('’', "'").replace(" '", " ")
    text_no_punctuation = re.sub(r'[^\w\s\'\-]', '', text_1)
    s = re.sub(r'\s+', ' ', text_no_punctuation).strip()
    text_lower = s.lower()
    return text_lower
...
def create_corpus_from_commonvoice(file):
    data = pd.read_csv(file, sep='\t')
    data.rename(columns={'path': 'clips'}, inplace=True)
    data['text'] = data["sentence"].apply(clean_commonvoice)
    data[['clips', 'text']].to_csv("dev_clean.csv", sep='\t', index=False)


if __name__ == '__main__':
    create_corpus_from_commonvoice("/data/macairec/PhD/Grammaire/corpus/csv/commonvoice/dev.tsv")
